...
def parse_color_argument(border_color_arg):
    if border_color_arg.lower() == 'transparent' or border_color_arg == "":
        return (0, 0, 0, 0)
    else:
...
def main():
    ...
    border_color = parse_color_argument(args.border_color if args.border_color is not None else input("Enter border color (transparent or hex code without #): "))
    
    create_google_maps_tiles(input_image_path, output_directory, min_zoom, max_zoom, border_color)

    # Compress all images in the selected directory
